...
| **Monotoniczność** | $x^1 \ge x^2 \implies x^1 \succeq x^2$ i $x^1 \gg x^2 \implies x^1 \succ x^2$ | "Więcej znaczy lepiej". | Krzywe obojętności mają nachylenie ujemne. Konsument wydaje cały dochód. |
| **Wypukłość** | $x^1 \succeq x^2 \implies tx^1 + (1-t)x^2 \succeq x^2$ dla $t \in $ | "Średnie są preferowane (nie gorsze) od skrajności". | Krzywe obojętności są wypukłe względem początku układu (malejący MRS). |
...
4.  **Rozwiązanie:** Dzieląc pierwsze równanie przez drugie, eliminujemy $\lambda$ i wspólny człon z funkcją użyteczności, co prowadzi do warunku styczności:
    $$ \frac{x_1^{\rho-1}}{x_2^{\rho-1}} = \frac{p_1}{p_2} \implies \left(\frac{x_1}{x_2}\right)^{\rho-1} = \frac{p_1}{p_2} $$

...
* **Jest wypukły**: Jeśli weźmiemy dwa dowolne koszyki ze zbioru $X$, to każdy koszyk będący ich **kombinacją wypukłą** (czyli ich średnią ważoną) również należy do tego zbioru. Jeśli dwa dowolne koszyki $x^1$ i $x^2$ należą do $X$, to każda ich kombinacja wypukła (średnia ważona) $x^t = tx^1 + (1-t)x^2$ dla $t \in [0,1]$ również należy do $X$. Oznacza to, że zbiór nie ma "dziur" ani "przerw". Jeśli konsumpcja dwóch skrajnych koszyków jest możliwa, to możliwa jest również konsumpcja każdego uśrednionego koszyka leżącego na odcinku, który je łączy.
...
Dla każdych $x^1, x^2 \in \mathbb{R}_{+}^{n}$, jeśli $x^1 \succeq x^2$, to dla każdego $t \in [0,1]$ zachodzi $tx^1 + (1-t)x^2 \succeq x^2$.
...
$$B = \{ x \in \mathbb{R}_{+}^{n} \mid p \cdot x \le y \}$$
...
$$ \frac{\partial u(x^*)}{\partial x_i} - \lambda p_i \le 0, \quad x_i^* \ge 0, \quad \text{oraz} \quad x_i^* \left( \frac{\partial u(x^*)}{\partial x_i} - \lambda p_i \right) = 0 $$

Dla większej czytelności (równoważnie):

$$
\begin{aligned}
&\frac{\partial u(x^*)}{\partial x_i} - \lambda p_i \le 0, \\
&x_i^* \ge 0, \\
&x_i^*\Big(\frac{\partial u(x^*)}{\partial x_i} - \lambda p_i\Big) = 0.
\end{aligned}
$$
